...
export async function renderAccountantsTable() {
  ...
  tbody.innerHTML = items.map((t) => {
    ...
    return `<tr>
      ...
      <td>${regStatus}</td>
      <td>${approveBtn}</td>
      <td>${actionBtns} ${activityBtn} ${unlinkBtn}</td>
    </tr>`;
  }).join('');
}
...
/**
 * Opens a modal showing every logged activity performed by an accountant —
 * login/logout, fee payments, income/expenditure records, debtors list
 * generation, password changes, reprints. Rows come from `staff_activities`.
 */
window.viewAccountantActivities = async function (accountantId) {
  ...
  getEl('staffActivitiesError').style.display = 'none';
  try {
    // Load the accountant header info
    const { data: accountant } = await supabaseClient.from('accountants')
    ...
    const { data: activities } = await supabaseClient.from('staff_activities')
      .select('*')
      .eq('staff_id', accountantId)
      .eq('staff_type', 'accountant')
      .order('created_at', { ascending: false })
      .limit(500);

    const body = getEl('staffActivitiesBody');
    const noAct = getEl('noStaffActivities');
    ...
    console.error('viewAccountantActivities error:', err);
    const errEl = getEl('staffActivitiesError');
    if (errEl) {
      errEl.textContent = 'Error loading activities: ' + err.message;
