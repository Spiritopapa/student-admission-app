...
import { getEl, showMessage, clearMessage, setLoading, getCurrentSchoolId, formatCurrency, formatDate, generateAcademicYearOptions, getDefaultAcademicYear, openPrintWindow } from './utils.js';
...
async function changeAccountantPassword(e) {
  ...
  if (np.length < 6) { showMessage('accountantProfileMessage', '6+ chars.', 'error'); setLoading(btn, false, 'Change Password'); return; }
  if (np !== cp) { showMessage('accountantProfileMessage', 'Mismatch.', 'error'); setLoading(btn, false, 'Change Password'); return; }
  try {
...
async function loadAccFeeOverview() {
  const schoolId = await _getSchoolId();
  try {
    let query = supabaseClient.from('fees').select('total_amount, amount_paid, debt, balance');
    if (schoolId) query = query.eq('school_id', schoolId);
