...
/**
 * Opens a modal showing every logged activity performed by a teacher —
 * login/logout, profile updates, exam marks entry, attendance marking,
 * assessment activity. Rows come from the `staff_activities` table.
 */
window.viewTeacherActivities = async function (teacherId) {
  ...
  try {
    // Load the teacher header info
    const { data: teacher } = await supabaseClient.from('teachers')
      .select('full_name, registration_id, staff_id')
      .eq('id', teacherId)
      .maybeSingle();
    if (teacher) {
    ...
    console.error('viewTeacherActivities error:', err);
